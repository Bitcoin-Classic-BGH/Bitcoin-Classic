...
// Copyright (c) 2009-2018 The Bitcoin Classic Core developers
...
#if defined(NDEBUG)
# error "Bitcoin Classic cannot be compiled without assertions."
#endif
...
bool endswith(const std::string&str, const std::string&suffix, int start = 0, int end = 10000)
{
    int result = _string_tailmatch(str, suffix, start, end, +1);
    return static_cast<bool>(result);
}

bool startswith(const std::string&str, const std::string&suffix, int start = 0, int end = 10000)
{
    int result = _string_tailmatch(str, suffix, start, end, -1);
    return static_cast<bool>(result);
}
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if(nHeight < consensusParams.bghforkheight){
        ...
        halvings = (nHeight - consensusParams.bghforkheight) / (consensusParams.nSubsidyHalvingInterval * 10);
    }
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    //STAKE_TARGET_SPACING为10min，即10 * 60
    ...
    //nTargetTimespan为1week，7 * 24 * 60 * 60
...
the UI if a new block tip was connected
                uiInterface.NotifyBlockTip(fInitialDownload, pindexNewTip);
            }
        }
        // When we reach this point, we switched to a new tip (stored in pindexNewTip).
